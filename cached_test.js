...
const neverOutOfBounds = (lat, lon) => false
const alwaysOutOfBounds = (lat, lon) => true

describe('land and sea', () => {
  it('in Venezuela, in the jungle', async () => {
    const latLon = { lat: 2.576, lon: -66.72 }
    const result = await get(api, neverOutOfBounds, latLon)
    assert(result)
  ...
  it('off the coast of India', async () => {
    ...
    const result = await get(api, alwaysOutOfBounds, latLon)
